...
library(plotly)
library(scales)
setwd("/Users/Asbarqi/Documents/Semester 2/R/Project")
```
...
The HPI is available for all European Union Member States (except Greece), the United Kingdom (only until the third quarter of 2020), Iceland, Norway, Switzerland and Türkiye.
...
ggplot(df_avg_grouped, aes(x = as.factor(Year), y = avg_HPI, color = refugee_influx_group)) +
  ...
  labs(
    title = " HPI Over Time (2010–2023) by Refugee Influx Groups",
    x = "Year",
...
ggplot(top_data, aes(x = Year, y = HPI, color = Country)) +
  ...
  theme_minimal(base_size = 13) +
  labs(
    title = "HPI Trends in Top 20 Refugee-Intake Countries (2014–2023)",
    x = "Year",
...
ggplot(data = europe_merged) +
  ...
  labs(
    title = "Refugee Influx Groups in Europe (2010–2023)",
...
# Step 2: Filtering base period (2010–2014) to compute average shares
base_df <- refugee_df %>%
